...
- Delegation: `@delegated(Irina)` at end of title — **Irina is the only person tasks are delegated to** (she is the sole direct report). Never tag `@delegated` for anyone else. When another person owes input or a deliverable, write it inline as a dependency instead (e.g. "needs input from Frank") — that is not a delegation.
...
- **Delegation:** Only Irina (the sole direct report) is delegated to — track via `@delegated(Irina)` and follow up on stale ones. Do not `@delegated` any other name; anyone else's involvement is an input dependency, noted inline (e.g. "needs input from Frank"), not a delegation.
