...
### Scheduled QA (Jun 2026) — Tomás, 42, recently divorced rebuilding finances with mixed debt and retirement accounts
Source: automated nightly code-level QA sweep. Persona traced onboarding → plan → accounts → scenarios → cash flow. Prod health: /api/health and /sign-in both returned 403 (Cloudflare/WAF layer — not app-level errors; deploy appears up). Findings below are new issues not covered by prior QA.

- [ ] CurrentPlanDrawer `handleSave` has no try/catch — `app/financial-planner/components/CurrentPlanDrawer.tsx:134-151`. Async save calls `onAppliedPlanSaved` and `onPlanSaved` without error handling; if either rejects the drawer never closes and the user gets no feedback. Wrap in try/catch with `toast.error()` fallback. (/qa scheduled 2026-06-14)
- [ ] HouseholdContext silently swallows member-load errors — `lib/contexts/HouseholdContext.tsx:40-41`. Catch block only logs to console; no error state exposed to consuming components. If the API call fails the member list appears empty with no explanation or retry. Add an `error` state to the context and surface a recoverable message in the UI. (/qa scheduled 2026-06-14)
- [ ] UpgradePromptModal subscribe button has no loading state — `app/components/UpgradePromptModal.tsx:31-65`. `handleSubscribe` makes async Clerk billing API calls with no spinner, disabled state, or text change. Users can double-click or assume nothing happened. Add `isSubscribing` state, disable the button, and show "Opening checkout…" while in flight. (/qa scheduled 2026-06-14)
- [ ] ErrorBoundary fallback uses hardcoded non-theme Tailwind colors — `app/components/ErrorBoundary.tsx:41-48`. Default error UI renders with `bg-gray-800/50`, `text-gray-100`, `text-gray-400`, `bg-emerald-600` — none adapt to light mode. Replace with theme tokens (`bg-theme-surface`, `text-theme-primary`, `text-theme-secondary`, `bg-[var(--color-accent)]`). (/qa scheduled 2026-06-14)
- [ ] CashFlowChart uses 8+ hardcoded hex colors in Recharts SVG — `app/financial-planner/components/CashFlowChart.tsx:268-355`. Fill/stroke values like `#ef4444`, `#10b981`, `#3b82f6`, `#064e3b` won't adapt to light mode or future theme changes. Replace with CSS variables (`var(--color-negative-fg)`, `var(--color-positive-fg)`, `var(--color-accent)`, etc.). (/qa scheduled 2026-06-14)
- [ ] TransactionsDrawer empty state has no inline CTA — `app/financial-planner/components/TransactionsDrawer.tsx:424-427`. Shows "No transactions found. Import a CSV or add transactions manually." as plain text; the Import/Add buttons only exist in the header toolbar. Add inline "Import CSV" and "Add Transaction" buttons so the empty state isn't a dead end. (/qa scheduled 2026-06-14)
- [ ] Toast dismiss button missing `aria-label` — `lib/contexts/ToastContext.tsx:91`. The X close button is icon-only with no accessible name. Add `aria-label="Dismiss"`. (/qa scheduled 2026-06-14)
- [ ] TimelineDrawer back button missing `aria-label` — `app/financial-planner/components/TimelineDrawer.tsx:921`. Icon-only back-arrow button has no accessible label for screen readers. Add `aria-label="Back to timeline"`. (/qa scheduled 2026-06-14)
