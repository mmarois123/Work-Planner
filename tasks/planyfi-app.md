...
### Scheduled QA (Jun 2026) — Tomás, recently divorced dad re-entering financial planning
Source: Automated nightly code-level QA sweep. Persona: Tomás, 41, recently divorced single dad with two kids, managing a 401(k) rollover and child-support payments, wants to model "when can I buy a house again?" via events/scenarios. Key journeys: onboarding → budget with child support → accounts (rolled-over 401k) → home purchase event → net worth projections. Production health: /api/health and /sign-in both returned 403 externally (likely WAF/CDN blocking automated requests — middleware correctly excludes health from auth).
- [ ] Onboarding fire-and-forget async calls silently lose data on failure — `void updateProfile()` (app/onboarding/page.tsx:263), `.then()` with no `.catch()` on account fetch (app/onboarding/components/OnboardingAccountsWrapper.tsx:24), no try-catch on `handleSkip` (page.tsx:349) or `OnboardingPlanWrapper.handleSave` (OnboardingPlanWrapper.tsx:16). If any API call fails, user data doesn't persist but navigation continues. Wrap all onboarding async ops in try-catch with toast.error feedback (/qa scheduled 2026-06-12)
- [ ] Onboarding AccountsStep account-toggle cards are keyboard-inaccessible — `role="button"` div (app/onboarding/components/AccountsStep.tsx:137) lacks `tabIndex={0}` and `onKeyDown` handler. Keyboard users cannot navigate to or activate toggles. Add tabIndex and Enter/Space handler (/qa scheduled 2026-06-12)
- [ ] TimelineDrawer `handleSaveEvent` doesn't clear `saving` state — save-button spinner can persist indefinitely after a successful custom-event save because `setSaving(false)` only runs in the separate `handleFormSave` finally-block, not in `handleSaveEvent` (app/financial-planner/components/TimelineDrawer.tsx:~418). Unify save-state cleanup in a single finally block (/qa scheduled 2026-06-12)
- [ ] Empty-state dead ends with no CTA — ComparisonChart.tsx (~line 95) shows "Add a scenario to compare projections" as plain text with no button; MortgageRefinanceForm.tsx (~line 144) shows "No mortgage accounts found — Create a Home Purchase event first" with no navigation action. Add CTA buttons to both (/qa scheduled 2026-06-12)
- [ ] Icon-only buttons missing aria-labels (~7 instances) — TimelineDrawer.tsx close/back buttons (lines 899, 908, 947), Toast.tsx close button (line 28), VarianceTracker.tsx month-navigation arrows (line 235). AccountsDrawer.tsx add/link/refresh buttons (lines 86–116) have Tooltip but no `aria-label` (tooltip is not a screen-reader substitute). Add aria-label to each (/qa scheduled 2026-06-12)
- [ ] Inline balance/name inputs missing aria-labels — AccountSetupWizard.tsx (~line 86) balance input, HoldingEditPopover.tsx (~line 133) holding-name input, and AccountRow.tsx (~line 61) inline balance editor are all announced as unlabeled by screen readers. Add `aria-label="Balance"` / `"Holding name"` / `"Balance amount"` respectively (/qa scheduled 2026-06-12)
- [ ] TargetAllocationDrawer Save disabled with no explanation — Save button (TargetAllocationDrawer.tsx ~line 126) is grayed out when allocation total ≠ 100% but no message tells the user why. Add inline validation text: "Total must equal 100% (currently X%)" (/qa scheduled 2026-06-12)
